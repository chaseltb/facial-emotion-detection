...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # Upgrade to kernel = 64
        x = self.batchNorm1(F.relu(self.conv1(x)))

        # Convolution, batch norm, pool, dropout at kernel=64
        x = self.batchNorm1(F.relu(self.conv2(x)))
        # Map size is halved (48x48)
        x = self.pool(x)
        x = self.dropout1(x)

        # Upgrade to kernel = 128
        x = self.batchNorm2(F.relu(self.conv3(x)))

        # Convolution, batch norm, pool, dropout at kernel = 128
        x = self.batchNorm2(F.relu(self.conv4(x)))
        # Map size is halved (24x24)
        x = self.pool(x)
        x = self.dropout2(x)

        # Upgrade to kernel = 256
        x = self.batchNorm3(F.relu(self.conv5(x)))

        # Convolution, batch norm, pool, dropout at kernel = 256
        x = self.batchNorm3(F.relu(self.conv6(x)))
        # Map size is halved (12x12)
        x = self.pool(x)
        x = self.dropout3(x)
        ...
        # And we go from 12*12*256 dimensions to 256
        x = F.relu(self.fc1(x))
        # To 128
        x = F.relu(self.fc2(x))
        # To 8
        x = self.fc3(x)
        # And softmax those 8 for our final values
        x = self.softmax(x)
        return x
...
def train():
    ...
    save_path = ".\\trained_models\\best-cnn.pt"
